...
export class UsersController {
  async createUser(req: IRequest, h: ResponseToolkit): Promise<ResponseObject | Boom> {
    try {
  ...
  async getUsers(req: IRequest, h: ResponseToolkit): Promise<ResponseObject | Boom> {
    try {
      if (req.params.id) {
        const user = await userModel.findById(req.params.id).lean({ autopopulate: true });
  ...
  async updateUser(req: IRequest, h: ResponseToolkit): Promise<ResponseObject | Boom> {
    try {
      const userPayload = req.payload as User;
      ...
      await userModel.updateOne({ _id: user._id }, { ...userPayload, id: user._id });
  ...
  async updateUserPassword(req: IRequest, h: ResponseToolkit): Promise<ResponseObject | Boom> {
    try {
      const userPayload = req.payload as User;
      ...
      if (user.password !== userPayload.oldPassword) return notFound("Old password doesn't match.");

      if (userPayload.newPassword !== userPayload.newPasswordConfirmation)
        return notFound("New password and new password confirmation doesn't match.");
  ...
  async updateUserActivationState(
    ...
    const user = await userModel.findById(decodedToken.id, 'isActive registrationToken ').lean();
  ...
  async deleteUsers(req: IRequest, h: ResponseToolkit): Promise<ResponseObject | Boom> {
    try {
